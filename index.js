...
app.post('/login', async (req, res) => {
    ...
    for (let i = 0; i < users.length; i++) {
        if (users[i][0] === id &&
            users[i][1].toLowerCase().trim() === first.toLowerCase().trim() &&
            users[i][2].toLowerCase().trim() === last.toLowerCase().trim()) {
            valid = true;
            row = i;
            break;
        }
...
app.get('/hrlog', checkLoggedIn, async (req, res) => {
    try {
        ...
        const { penaltyHours, m1, m2, m3 } = req.session.user;

        res.render('hrlog', { user: req.session.user, entries, totalHours, penaltyHours, m1, m2, m3 });
    } catch (err) {
...
app.get('/faq', checkLoggedIn, (req, res) => res.render('faq'));
